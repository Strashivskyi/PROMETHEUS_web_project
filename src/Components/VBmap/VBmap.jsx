...
function About() {
    ...
    const handleUpload = () => {
        const uploadTask = storage.ref(`patients/${localStorage.getItem('child')}/vbmap/${filename.name}`).put(filename);
        uploadTask.on("state_changed", snapshot => {
            }, error => {
                console.log(error)
            },
            () => {
                storage.ref('patients/' + localStorage.getItem('child') + "/vbmap").child(filename.name).getDownloadURL().then(url => {
                    // console.log(url)
