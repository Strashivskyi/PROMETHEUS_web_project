...
let test=[]
    ...
    useEffect(() => {
        const db = app.firestore();
        const unsubscribe = db.collection("User").doc(localStorage.getItem("user")).collection("Patient").where("Name", "==", localStorage.getItem("childName"))
            .onSnapshot(snapshot => {
                if (snapshot.size) {

                    setChild(snapshot.docs.map(doc => ({ ...doc.data(), id: doc.id })));
                    console.log("Сука")
                } else {
                    console.log("Сука1")
                }
